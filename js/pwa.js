...
// if ('serviceWorker' in navigator) {
// 	navigator.serviceWorker.register('./sw.js')
// 		.then(registration => console.log('[Page]: The Serviceworker was registered.',registration.scope))
// 		.catch(err => console.error('[Page]: Not able to register ServiceWorker.', err.scope))
// }
...
$(function () {
  ...
  setTimeout(function () {
      $('.pwa-dismiss').on('click', function () {
          ...
          createCookie('Homepage_pwa_rejected_install', true, 1);
      ...
      function showInstallPromotion() {
          if (!$('body').hasClass('is-installed')) {
              if ($('#menu-install-pwa-android, .add-to-home').length) {
                  ...
                  if (!readCookie('Homempage_pwa_rejected_install')) {
                      setTimeout(function () {
  ...
  if (isMobile.iOS()) {
      if (!isInWebAppiOS) {
          ...
          if ($('#menu-install-pwa-ios, .add-to-home').length) {
              if (!readCookie('Homepage_pwa_rejected_install')) {
  ...
  if (pwaCookie == false) {
      eraseCookie('Homepage_pwa_rejected_install');
  }
